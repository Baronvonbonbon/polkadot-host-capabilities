...
- `host.cloud.upload` — **Retracted, a probe bug:** Not measured: sonde's spend gate refused it. The host resolved only Paseo Asset Hub's chain spec (0xd6eec261…), which sonde's allowlist did not hold. Fixed in sonde on 2026-09-19. The run said: REFUSED: genesis 0xd6eec261… is not in SPEND_ALLOWED_GENESIS. Add it to product.mjs only after confirming it is a chain you are willing to spend on.
- `host.cloud.roundTrip` — **Retracted, a probe bug:** Not measured: needs host.cloud.upload, which sonde's spend gate refused. The run said: Needs "host.cloud.upload", which came back skip. Running anyway would test a placeholder.
- `host.cloud.read` — Fetched 0.1 KiB in 153 ms and the CID verifies. Reads work.
