...
- `host.cloud.read` — Fetched 0.1 KiB in 245 ms and the CID verifies. Reads work.
- `host.preimage.lookup` — Lookup never called back in 5 s — neither a hit nor a miss.
  ...
  ```
- `host.cloud.seal` — Sealed a placeholder in 1 ms (run the camera probe first for a real payload).
