...
- `host.statement.submit` — **Retracted, a probe bug:** Not measured: refused because no genesis resolved (the host.chain.genesis probe bug) and sonde's spend allowlist was empty. Both fixed in sonde on 2026-09-19. The run said: No genesis hash resolved yet — run host.chain.genesis first. Spending on an unidentified chain is refused.
